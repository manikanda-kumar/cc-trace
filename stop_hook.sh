#!/bin/bash
###
# Claude Code Stop Hook - LangSmith Tracing Integration with Project Metadata
# Sends Claude Code traces to LangSmith after each response.
# Enhanced to include GitHub project name and folder name in trace metadata.
###
...
get_github_project_name() {
    ...
    local project_name
    project_name=$(echo "$remote_url" | sed -E 's|.*/([^/]+)(\.git)?$|\1|')
    echo "$project_name"
}
...
# Project metadata is derived from hook input inside main() to avoid consuming stdin twice.
# These are populated before create_trace() is called.
PROJECT_DIR=""
CWD_DIR=""
FOLDER_NAME=""      # repo-relative folder path ("." or "sub/dir")
FOLDER_TAG=""       # folder path with '/' replaced for tags
GITHUB_PROJECT=""   # repo name (derived from git remote)
...
send_multipart_batch() {
    ...
    local run_ids
    run_ids=$(echo "$batch_json" | jq -r '.[].id' 2>/dev/null | head -n 5 | paste -sd, -)
    log "INFO" "Sending $operation batch: size=$batch_size ids=${run_ids:-none}"
...
create_trace() {
    ...
    turn_data=$(jq -n \
        ...
        --arg github_project "$GITHUB_PROJECT" \
        ...
        '{
            ...
            tags: ["claude-code", ("turn-" + $turn), $github_project, $folder_name]
        }')
...
main() {
    ...
    # Compute repo + folder metadata once per hook invocation
    local cwd
    cwd=$(echo "$hook_input" | jq -r '.cwd // ""')
    CWD_DIR="$cwd"
    PROJECT_DIR="${CLAUDE_PROJECT_DIR:-${cwd:-$PWD}}"

    # repo name from git remote at project root
    GITHUB_PROJECT=$(get_github_project_name "$PROJECT_DIR")

    # folder relative to repo root (prefer cwd)
    if [ -n "$cwd" ] && [[ "$cwd" == "$PROJECT_DIR"* ]]; then
        FOLDER_NAME="${cwd#"$PROJECT_DIR"}"
        FOLDER_NAME="${FOLDER_NAME#/}"
        if [ -z "$FOLDER_NAME" ]; then
            FOLDER_NAME="."
        fi
    else
        FOLDER_NAME=$(get_folder_name "${cwd:-$PROJECT_DIR}")
    fi
    FOLDER_TAG="${FOLDER_NAME//\//_}"

    log "INFO" "Resolved metadata: repo=$GITHUB_PROJECT folder=$FOLDER_NAME project_dir=$PROJECT_DIR cwd=$CWD_DIR"
    if [ "$GITHUB_PROJECT" = "unknown" ]; then
        log "WARN" "Git remote not detected; traces will use repo=unknown"
    fi

    # Check stop_hook_active flag
    if echo "$hook_input" | jq -e '.stop_hook_active == true' > /dev/null 2>&1; then
        log "INFO" "stop_hook_active=true, skipping LangSmith upload for session"
        exit 0
    fi
    ...
    log "INFO" "Processing session $session_id (transcript=$transcript_path)"
    ...
    if [ -z "$new_messages" ]; then
        log "INFO" "No new messages to trace for session $session_id (last_line=$last_line)"
        exit 0
    fi
